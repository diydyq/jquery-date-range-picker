...
}(function ($, moment)
{
	...
	$.fn.dateRangePicker = function(opt)
	{
		...
		function init_datepicker()
		{
			...
			//// Hide daterangepicker if click other place.
			//$(document).mousedown(function (evt) {
			//	var target = evt.target;
			//	if(box.find(target).length === 0){
			//		closeDatePicker();
			//	}
			//});
		...
		function calcPosition()
		{
			if (!opt.inline)
			{
				...
				else
				{
					...
					// Planned width
					var plannedWidth = offset.left + $(box).width();

					if(plannedWidth < containerWidth){
						// Planned position.
						box.css(
							{
						...
						box.css(
							{
								top: 	offset.top+$(self).outerHeight() + parseInt($('body').css('border-top') || 0,10 ),
								left: 	containerWidth - $(box).width() - 10
							});
					}
		...
		function showMonth(date,month)
		{
			...
			var tmpl = '<span class="year">$0</span> <span class="month">$1</span>'.replace('$0', date.getFullYear()).replace('$1', monthName);
			box.find('.'+month+' .month-name').html(tmpl);
		...
		function showGap()
		{
			...
			box.find('.gap').height(Math.min(h1,h2) - 10);
		}
		...
		function createDom()
		{
			...
			if (opt.showTopbar)
			{
				...
				html += '<div class="error-top">error</div>\
						...
						<input type="button" class="btn apply-btn disabled'+ getApplyBtnClass() +'" value="'+lang('apply')+'" />';
				html += '</div>'
			}
				...
				if (data)
				{
					if (data['prev-days'] && data['prev-days'].length > 0)
					{
						...
						for(var i=0;i<data['prev-days'].length; i++)
						{
							...
							name = lang('Past') + name + ((data['prev-days'][i] > 1) ? lang('days') : lang('day'));
							var aLabel = lang(name) ? lang(name) : name;
							html += ' <a href="javascript:;" shortcut="day,-'+data['prev-days'][i]+'">' + aLabel + '</a>';
						}
						html+='</span>';
					}
					...
					if (data['next-days'] && data['next-days'].length > 0)
					{
						...
						for(var i=0;i<data['next-days'].length; i++)
						{
							...
							name = lang('Next') + name + ((data['next-days'][i] > 1) ? lang('days') : lang('day'));
							var aLabel = lang(name) ? lang(name) : name;
							html += ' <a href="javascript:;" shortcut="day,'+data['next-days'][i]+'">'+ aLabel +'</a>';
						}
						html+= '</span>';
					}
